...
export class HeaderComponent implements OnInit {
  ...
  constructor(router: Router,
    ...
    router.events.forEach((event) => {
      if (event instanceof NavigationEnd) {
        // var tech = activatedRoute.data.pipe(map(data => data.header));
        this.header = 'My header!!';
      }
    });
    this.appService.getTicket().subscribe(
      x => {
        this.ticket = x;
      }
    );
  ...
  logout() {
    window.location.href = this.baseUrlServiceService.getBase() + '/logout';
  }
