...
export class BaseUrlServiceService {
  ...
  public getPath() {
    if (this.path === undefined) {
      if (location.pathname.indexOf('/safari') !== -1) {
        this.path = location.pathname.split('/safari')[0] + '/safari';
      } else {
        this.path = '';
      }
    }
  ...
  public getWebSocketUrl() {
    ...
    return wsProtocol + '//' + this.getHost() + ':' + this.getPort() + this.getPath() + '/ws';
  }

  public getBase() {
    return location.protocol + '//' + this.getHost() + ':' + this.getPort() + this.getPath();
  }

  public getRestApiBase() {
